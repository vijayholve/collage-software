...
from django.db.models import Q  
from students.models import CustomUser ,Teacher,Student,Subject,hod  ,Test,TestResult ,ClassGroup
...
def user_panel(request,user_id ,user_type):
    ...
    if user_type == 'student':
        ...
        if request.method == "POST":
            ...
            try:
                clgroup_detail=ClassGroup.objects.get(id=classgroup)
            except:
                messages.error(request,"please select a correct classroon ")
                return redirect('user-type',user_id=user_id,user_type)
            try:
                std=Student.objects.get(roll_no=rollno,classgroup=classgroup)
            except:
                messages.error(request,"please select a correct classroon and enter a corect roll no ")
            if std:
