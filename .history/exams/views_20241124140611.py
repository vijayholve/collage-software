...
def take_test(request, test_id):
    ...
    student =student request.user  # Assuming the student is the logged-in user
    ...
    if request.method == 'POST':
        ...
        student_score, created = StudentScore.objects.get_or_create(
            ...
            is_passed=True if score>= test.passing_marks else False ,
        )
        ...
        if not created:
            ...
            if test.passing_marks:
                student_score.is_passed=True if score >= test.passing_marks else False
            else:
                student_score.is_passed=True
            student_score.save()
...
def students_result_exam(request,test_id):
    ...
    no_of_quistion=Question.objects.filter(test=test).count
    students_details = {} 
