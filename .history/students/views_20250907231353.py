...
def capture_and_recognize(request):
    ...
    def capture_and_recognize(request):
        ...
        cap = None
        win_name = "Recognition - Default Camera"
        try:
            cap = cv2.VideoCapture(0)  # Use default laptop camera
            if not cap.isOpened():
                raise Exception("Cannot open default camera.")
            ...
            while True:
                ...
                if test_encs and known_encs:
                    results = recognize_faces(np.array(known_encs), known_names, test_encs)
                    detected_boxes = mtcnn.detect(rgb)[0]
                    if detected_boxes is not None:
                        for name, box in zip(results, detected_boxes):
                            if box is not None:
