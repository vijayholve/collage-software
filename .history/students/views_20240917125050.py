...
def particular_student_attendance(request, sid): 
    ...
    end_date = today  
    holidays=Holiday.objects.filter(dates__range=[start_date,end_date])
    ...
    attendance_data = {holiday = holidays.filter(date=date).first() if date: Attendance.objects.filter(student=student, date=date).exists()
