...
def particular_student_attendance(request,sid):
    ...
    attendance_data={date: Attendance.objects.filter(student=student, date=date) 
            for date in (start_date + timedelta(days=i) for i in range(day))
    }
    dates = [start_date + timedelta(days=i) for i in range(day)]
    context = {
        'student': student,
    ...
    return render(request, 'students/students_attendance_table.html', context) 
