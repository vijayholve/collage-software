...
def attendance_data_students(request, pk):
    ...
    # Create a dictionary to store the attendance or holiday info for each student
    attendance_data = {}
    for student in students:
        ...
        for date in (start_date + timedelta(days=i) for i in range(7)):
            ...
            if holiday:
                student_attendance[date] = f"Holiday - {holiday.name}"  # Mark it as holiday
            else:
    ...
    dates = [start_date + timedelta(days=i) for i in range(7)]  # Generate the last 7 dates
