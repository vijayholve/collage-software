...
def create_timetable_of_class(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect('timetable_list',classid=form.instance.classgroup.id)
    content={"form":form}
    return render(request,"timetables/create_timetables.html",content)
...
def timetable_list(request,classid):days_of_week = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
    time_slots = TimeSlot.objects.all().order_by('start_time')
    timetable = {day: {slot: [] for slot in time_slots} for day in days_of_week}
    
    for entry in Timetable.objects.all():
        timetable[entry.time_slot.day_of_week][entry.time_slot].append(entry)
    ...
    return render(request, 'timetables/timetable_list.html', {'timetables': timetables, 'days_of_week': days_of_week})