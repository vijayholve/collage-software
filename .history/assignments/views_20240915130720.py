...
def check_assignment(request,studentid,assignementid):
    student = get_object_or_404(Student, id=id)
    today = datetime.now().date()
    assignment=Assignment.objects.get(id=assignementid)
    assgnemnt_subm, created = AssignmentSubmission.objects.get_or_create(student=student,assignment=assignment,submitted_date=today)
