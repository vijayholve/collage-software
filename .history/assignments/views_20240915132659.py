...
def check_assignment(request, studentid, assignmentid):
    ...
    assgnemnt_subm, created = AssignmentSubmission.objects.get_or_create(student=student, assignment=assignment,
                                                                         status=True,submitted_date = today)
    assgnemnt_subm.status = True
    assgnemnt_subm.submitted_date = today
    assgnemnt_subm.save()
    
    # Return JSON response instead of redirecting
    return JsonResponse({"success": True, "message": "Assignment marked as submitted!"})