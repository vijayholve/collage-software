...
def all_classes(request):
    ...
    years = ClassGcoup.objects.filter(name=class_ba)
...
def export_assignment_data_excel(request, class_id):
    classgroup = ClassGroup.objects.get(id=class_id)
    assignments = Assignment.objects.filter(classgroup=classgroup)
    ...
    for student in students:
        ...
        for ass in assignments:
            ...
            assignement_exists=AssignmentSubmission.objects.filter(
                Q(student=student, assignment=ass)
            ).exists()
            row[ass] = 'Complete' if assignement_exists  else '❎'
        data.append(row)
    
    df = pd.DataFrame(data)
    
    # Create an Excel writer using pandas with openpyxl engine
    file_path = rf'students\management\assignments:-{classgroup.name}-{classgroup.year}.xlsx'
    with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
        df.to_excel(writer, index=False, sheet_name=f'{classgroup.name}-{classgroup.year} assignments')
