...
@login_required(login_url="login-page")
def assignement_student_home(request,sid):
    student=Student.objects.get(id=sid)
    classgroup=student.classgroup 
    assignments=Assignment.objects.filter(classgroup=classgroup) 
    assignment_status_student=AssignmentSubmission.objects.filter(
        assignment=assignments,
        student=student
    ).exists()
    
    content={'assignments': assignments,"assignment_status_student"} 
    return render(request,rf'assignements\assignemts_home.html',content)
