...
def feedback_by_category(request, category_id):
    ...
    responses = FeedbackResponse.objects.filter(question__category=category)
    content=
    return render(request, 'feedback_by_category.html',content)
...
def add_question(request):
    if request.method == 'POST':
        ...
        if question_form.is_valid():
            ...
            return redirect('list-feedbacks')  # Redirect to a page showing the list of questions
    else:
    ...
    if request.method == 'POST':
        ...
        for question in FeedbackQuestion.objects.all():
            ...
            if question.question_type == 'text':
                FeedbackResponse.objects.create(question=question, user=user, response_text=response_text)
            elif question.question_type == 'mcq':
                FeedbackResponse.objects.create(question=question, user=user, response_choice=response_text)
            elif question.question_type == 'likert':
                FeedbackResponse.objects.create(question=question, user=user, response_likert=int(response_text))
        return redirect('feedback_thank_you')
    else:
        questions = FeedbackQuestion.objects.all()
        return render(request, 'feedbacks/submit_feedback.html', {'questions': questions})
